    ...
    return (
        <tr>
            ...
            <td style={{minWidth: '200px'}} className="w-50 align-middle" >
                <h5 className="text-capitalize text-secondary font-weight-bold">
                    <Link href={`/product/${item._id}`}>
                        <a>{item.title}</a>
                    </Link>
                </h5>

                <h6 className="text-danger font-weight-bold">${item.quantity * item.price}</h6>
                {
                    item.inStock > 0
                    ? <p className="mb-1 text-danger font-weight-bold">In stock: {item.inStock}</p>
                    : <p className="mb-1 text-danger font-weight-bold">Out stock</p>
                }
            </td>
